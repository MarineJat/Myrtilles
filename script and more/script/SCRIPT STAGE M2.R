...
d$nbframe_plotCOUNT<-d$number.of.frames.per.plot..count
d$nbflowers<-d$number.of.flowers..count.
d$BUB<-as.factor(d$browsed..boolean.)
...
unique(d$NaTron_datasetName) ##the different regions
d<-droplevels(d[d$NaTron_datasetName=="SUSTHERB Tingvoll",])  ## choose the region

# looking at the file to remove the columns with NA
summary(d)

# creating a smaller file with column of interest and short column names

d2<-d[,c("adminNames","shortloc","NEWloc","siteNumber","SiteStation","SiteStationYear","experiment","year",
         "recordNumber","BUB","scientificName","nbframe_plotCOUNT","shootlg")]
str(d2)
...
### NB : distribution of he nb of record (should be 10...)
table(d$recordNumber) # goes up to 36
plot(table(d$recordNumber))
...
# Utiliser les mêmes noms de colonnes que vous avez sélectionnés
bubshootlength <- d %>% select(experiment, shoot.length..centimeter.)

# Renommer les colonnes pour correspondre à celles que vous utilisez dans ggplot2
...
tablongilati$decimalLatitude2 <- as.numeric(gsub(",",".", tablongilati$decimalLatitude))
tablongilati$decimalLongitude2 <- as.numeric(gsub(",",".", tablongilati$decimalLongitude))

# Select numeric variables for correlation analysis
tablongilati <- tablongilati[,c("decimalLatitude2", "decimalLongitude2", "shoot.length..centimeter.")]
...
tabregion = d%>%select("NaTron_datasetName", "year", "shoot.length..centimeter.")
############ CETTE LIGNE NE MARCHE PLUS JE COMPRENDS PAS ?????????
...
# Creation of a new table with the 3 columns
tabBUB <- d %>% select("experiment", "year", "shoot.length..centimeter.")
...
cor(d$shoot.length..centimeter., d$year, method="pearson")
...
complete_shootStation <- d[complete.cases(d$shoot.length..centimeter., d$stationNumber), ] # Exclude observations with missing values in both variables
...
d$shoot.length..centimeter.[is.na(d$shoot.length..centimeter.)] <- mean(d$shoot.length..centimeter., na.rm = TRUE)

# Check if there are any missing values left
sum(is.na(d$shoot.length..centimeter.))
...
model_site <- lm(shoot.length..centimeter. ~ siteNumber, data = d)
...
modele_locality <- lm(shoot.length..centimeter. ~ locality, data = d)
...
modele_NaTron_datasetName <- lm(shoot.length..centimeter. ~ NaTron_datasetName, data = d)
...
modele_experiment <- lm(shoot.length..centimeter. ~ experiment, data = d)
...
modele_stationNumber <- lm(shoot.length..centimeter. ~ stationNumber, data = d)
...
modele_year <- lm(shoot.length..centimeter. ~ year, data = d)
...
modele_multiple <- lm(shoot.length..centimeter. ~ siteNumber + locality + NaTron_datasetName + experiment + stationNumber + year, data = d)
...
# Data
Longilati <- data.frame(
  decimalLatitude = rnorm(100),
  decimalLongitude = rnorm(100),
  number.of.frames.per.plot..count.= rnorm(100)
)
